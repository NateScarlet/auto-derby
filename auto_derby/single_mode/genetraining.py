...
class g:
    training_class: Type[Genetraining]
...
def _ocr_training_effect(img: Image) -> int:
    ...
    fill_img = np.repeat(np.expand_dims(fill_gradient, 1),
                         cv_img.shape[1], axis=1)
...
def _recognize_level(rgb_color: Tuple[int, ...]) -> int:
    ...
    if imagetools.compare_color((165, 78, 255), rgb_color) > 0.9:
        return 5
    raise ValueError(
        "_recognize_level: unknown level color: %s" % (rgb_color,))
...
class Genetraining:
    ...
    @classmethod
    def from_training_scene(cls, img: Image) -> Genetraining:
        ...
        t, b = 503, 532
        self.speed = _ocr_training_effect(
            img.crop(rp.vector4((18, t, 91, b), 466)))
        self.stamina = _ocr_training_effect(
            img.crop(rp.vector4((91, t, 163, b), 466)))
        self.power = _ocr_training_effect(
            img.crop(rp.vector4((163, t, 237, b), 466)))
        self.guts = _ocr_training_effect(
            img.crop(rp.vector4((237, t, 309, b), 466)))
        self.wisdom = _ocr_training_effect(
            img.crop(rp.vector4((309, t, 382, b), 466)))
        self.skill = _ocr_training_effect(
            img.crop(rp.vector4((387, t, 450, b), 466)))
        return self
    ...
    def score(self, ctx: Context) -> float:
        ...
        pow = mathtools.integrate(
            ...
            (
                (0, 1.5),
                (300, 0.8 + ctx.speed / 600),
                (600, 0.2 + ctx.speed / 900),
                (900, ctx.speed / 900 / 3),
            ),
        )
        per = mathtools.integrate(
            ctx.guts,
            self.guts,
            ((0, 1.0), (300, 0.5), (400, 0.15), (600, 0.05))
        ...
        int_ = mathtools.integrate(
            ctx.wisdom,
            self.wisdom,
            ((0, 1.5), (300, 0.5), (400, 0.2), (600, 0.1))
            if ctx.vitality < 0.9
            else ((0, 2.0), (300, 0.8), (400, 0.1)),
        )

        if (ctx.date[0] == 2 and ctx.stamina < 450):
            sta *= 2
            spd *= 0.7
            pow *= 0.7

        if (ctx.date[0] == 3 and ctx.stamina < 600):
            sta *= 2
            spd *= 0.7
            pow *= 0.7
