...
def test_update_by_character_detail_8():
    img, _ = _test.use_screenshot("single_mode/character_detail_8.png")
    ctx = Context.new()
    ctx.update_by_character_detail(img)
    ...
    assert ctx.lead == ctx.STATUS_A, ctx.lead
    assert ctx.head == ctx.STATUS_B, ctx.head
    assert ctx.middle == ctx.STATUS_D, ctx.middle
    assert ctx.last == ctx.STATUS_E, ctx.last

    assert ctx.conditions == set((ctx.CONDITION_SHARP,)), ctx.conditions
